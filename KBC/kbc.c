...
unsigned __stdcall timer_function(void* seconds_ptr) {
    int seconds = *(int*)seconds_ptr;
    for (int i = 0; i < seconds * 10; i++) {
        Sleep(100);
        if (input_received) return 0;
    }
    timeout_happened = 1;
...
void play_game(Question* questions, int no_of_questions) {
  ...
  for (int i = 0; i < no_of_questions; i++) {
    ...
    timeout_happened = 0;
    unsigned threadID;
    HANDLE timerThread = (HANDLE)_beginthreadex(NULL, 0, &timer_function, &questions[i].timeout, 0, &threadID);
